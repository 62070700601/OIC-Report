...
GROUP BY PPMS_T_DEPARTMENT.NAME,TO_CHAR(DATE_OF_CONSULT,'YYYY'), "user".SALUTATION_NAME || ' ' || "user".FIRST_NAME || ' ' || "user".LAST_NAME
ORDER BY PPMS_T_DEPARTMENT.NAME,TO_CHAR(DATE_OF_CONSULT,'YYYY'), consult_total DESC, "user".SALUTATION_NAME || ' ' || "user".FIRST_NAME || ' ' || "user".LAST_NAME, count(PPMS_T_CONSULT.id) DESC